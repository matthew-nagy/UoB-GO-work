...
func parallelMergeSort(slice []int32, group *sync.WaitGroup) {
	defer group.Done()
	...
	if len(slice) > 1{
		...
		if len(slice) > 1000{
			...
			wg.Add(2)
			go parallelMergeSort(slice[:middle], &wg)
			parallelMergeSort(slice[middle:], &wg)
			wg.Wait()
		}else{
			mergeSort(slice[:middle])
			mergeSort(slice[middle:])
